...
def log_epoch_metrics(epoch: int, epochs: int, train_loss: float, train_acc: float, train_found_balls: float, val_loss: float, val_acc: float, val_found_balls: float, writer: SummaryWriter, csv_file: TextIO) -> None:
...
def save_model_checkpoints(model: torch.nn.Module, model_dir: str, epoch: int, val_loss: float, val_found_balls: float, best_val_loss: float, best_val_found_balls: float) -> tuple[float, float, int]:
...
def run_training_loop(model: torch.nn.Module, train_loader: torch.utils.data.DataLoader, test_loader: torch.utils.data.DataLoader, optimizer: torch.optim.Optimizer, criterion: torch.nn.Module, scheduler: torch.optim.lr_scheduler.ReduceLROnPlateau, writer: SummaryWriter, csv_file: TextIO, model_dir: str, device: torch.device, epochs: int = 10000) -> None:
    ...
    for epoch in range(epochs):
        ...
        best_val_loss, best_val_found_balls, patience_counter = save_model_checkpoints(model, model_dir, epoch, val_loss, val_found_balls, best_val_loss, best_val_found_balls)
