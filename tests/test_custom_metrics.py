...
from src.custom_metrics import FoundBallMetric
...
def test_update_state_perfect_prediction() -> None:
    ...
    with patch('src.custom_metrics.unscale_x', side_effect=lambda x: x), \
...
def test_update_state_calls_unscale_functions() -> None:
    ...
    with patch('src.custom_metrics.unscale_x') as mock_unscale_x, \
...
def test_update_state_tensor_conversion() -> None:
    ...
    with patch('src.custom_metrics.unscale_x', return_value=0.0), \
