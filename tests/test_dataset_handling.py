...
from src.dataset_handling import (
...
def test_downsample_by_averaging() -> None:
    ...
    expected = np.array([[[2.0, 2.0, 2.0], [3.0, 3.0, 3.0]],
                        [[4.0, 4.0, 4.0], [5.0, 5.0, 5.0]]], dtype=np.float32)
...
def test_load_image_basic() -> None:
    ...
    try:
        with patch('src.dataset_handling.img_scaled_height', 120), \
...
def test_crop_image_by_image() -> None:
    ...
    with patch('src.dataset_handling.img_scaled_width', 160), \
...
def test_calculate_random_crop_bounds() -> None:
    ...
    with patch('src.dataset_handling.patch_width', 40), \
         patch('src.dataset_handling.patch_height', 30):
...
def test_ball_dataset_getitem_train() -> None:
    ...
    try:
        with patch('src.dataset_handling.crop_image_random_with_ball') as mock_crop, \
...
def test_ball_dataset_getitem_test() -> None:
    ...
    try:
        with patch('src.dataset_handling.crop_image_by_image') as mock_crop, \
